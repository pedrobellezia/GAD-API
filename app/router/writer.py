...
async def route_post_writer(writer_data: WriterCreate, db: AsyncSession = get_db()):
    new_writer = await create_writer(db, writer_data)
    return new_writer


@router.get("/", response_model=list[WriterRead] | None)
async def route_get_writers(writer_data: WriterFilter, db: AsyncSession = get_db()):
...
async def route_get_writer_by_id(writer_id: UUID, db: AsyncSession = get_db()):
    new_writer = await get_writers(db, WriterFilter(id=writer_id).model_dump())
