...
async def route_post_client(client_data: ClientCreate, db: AsyncSession = get_db()):
    new_client = await create_client(db, client_data)
    return new_client


@router.get("/", response_model=list[ClientRead] | None)
async def route_get_clients(client_data: ClientFilter, db: AsyncSession = get_db()):
...
async def route_get_client_by_id(client_id: UUID, db: AsyncSession = get_db()):
